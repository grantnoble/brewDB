...
<html>
<head>
    <title>Fermentables XML Load</title>
</head>
<body>
...
/* 
fermentables_xml_load.php
Load fermentables details from an XML file into the database
*/
...
for ($i=0; $i<=count($array)-1; $i++)
  ...
  elseif ($array[$i]['level'] == 3)
	{
	if (($array[$i]['tag'] == 'NAME') || ($array[$i]['tag'] == 'VERSION') || ($array[$i]['tag'] == 'TYPE') || ($array[$i]['tag'] == 'YIELD') ||
		 ($array[$i]['tag'] == 'COLOR') || ($array[$i]['tag'] == 'ADD_AFTER_BOIL') || ($array[$i]['tag'] == 'ORIGIN') ||
		 ($array[$i]['tag'] == 'SUPPLIER') || ($array[$i]['tag'] == 'NOTES') || ($array[$i]['tag'] == 'MAX_IN_BATCH') ||
		 ($array[$i]['tag'] == 'RECOMMEND_MASH'))
    {
    $column .= $prefix;
